...
void gInicializaNivel(unsigned int semente)
{
    ...
    for (i = 1; i < NUM_PLATAFORMAS - 1; i++) {
        ...
        /* estrelas concentradas na primeira metade do percurso: a chance de
         * estrela so e sorteada para indices <= metade das plataformas
         * intermediarias, com probabilidade maior para compensar a janela
         * menor (mantem a quantidade total de estrelas similar a antes). */
        p->tem_estrela = 0;
        if (i <= (NUM_PLATAFORMAS - 1) / 2 &&
            (rand() % 100) < 45 && g_total_estrelas < NUM_ESTRELAS) {
            p->tem_estrela = 1;
...
void gDesenha(void)
{
    ...
    if (g_estado == ESTADO_MENU) {
        ...
        gTextoCentro(g_altura * 0.64f, "PLATFORMER 3D - Endrick - em busca da titularidade");
...
void gDesenhaJogador(void)
{
    ...
    glPushMatrix();
        ...
        /* pequeno detalhe verde no peito (estilizado, nao e o brasao oficial) */
        gCaixa(-0.07f, 0.78f + bob, -0.165f, 0.07f, 0.92f + bob, -0.155f);
 
        /* ---- cabeca (cubica, proporcionalmente grande) ---- */
        glColor3f(0.62f, 0.42f, 0.28f);                       /* pele morena */
        gCaixa(-0.26f, 1.10f + bob, -0.26f,  0.26f, 1.62f + bob, 0.26f);
 
        /* "cabelo": camada escura cobrindo o topo/tras da cabeca */
        glColor3f(0.08f, 0.06f, 0.05f);                       /* preto      */
        gCaixa(-0.27f, 1.50f + bob, -0.27f,  0.27f, 1.64f + bob, 0.27f);
        gCaixa(-0.27f, 1.30f + bob,  0.16f,  0.27f, 1.60f + bob, 0.27f);
        ...
        /* ---- pernas ("calca") ---- */
        glColor3f(0.15f, 0.20f, 0.45f);                       /* azul       */
...
void gDesenhaHUD(void)
{
    ...
    if (g_estado == ESTADO_VITORIA) {
        ...
        gTextoCentro(g_altura * 0.62f, "GOAL!  VOCE VENCEU!");
        glColor3f(0.6f, 1.0f, 0.7f);
        gTextoCentro(g_altura * 0.55f, "PARABENS, CAMPEAO! O TECNICO DISSE QUE");
        gTextoCentro(g_altura * 0.50f, "VOCE VAI SER TITULAR NA PROXIMA COPA - ATE 2030!");
        glColor3f(1.0f, 1.0f, 1.0f);
        sprintf(buf, "Pontuacao final: %d", g_pontos);
        gTextoCentro(g_altura * 0.42f, buf);
        gTextoCentro(g_altura * 0.34f, "R = jogar de novo (nova seed)");
